  ...
  <section v-if="activeMode === 'geometry'" class="border-t border-slate-100 bg-white px-4 py-4 space-y-3">
    <div class="flex items-start justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-[11px] leading-5 text-slate-400">为 point / line / segment 建立可解释关系，并随拖拽实时校验。</p>
      </div>
...
const relationHint = computed(() => {
  switch (selectedKind.value) {
    case 'parallel':
      return '需要两条 line-like / segment-like 目标，检查它们是否保持平行。';
    case 'perpendicular':
      return '需要两条 line-like / segment-like 目标，检查是否维持 90°。';
    case 'equal-length':
      return '需要两条线段目标，比较当前长度差。';
    case 'distance-assertion':
      return '需要两个点目标，判断当前距离是否满足目标值。';
    default:
      return '';
  }
});
