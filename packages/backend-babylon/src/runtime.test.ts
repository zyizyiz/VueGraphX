...
describe('BabylonRuntime', () => {
  ...
  it('ignores legacy 2D render mode requests and keeps the runtime in 3D mode', () => {
    const runtime = createBabylonRuntime(createFakeBabylon(), {
      renderMode: '2d' as never,
    ...
    expect(FakeCamera.last?.mode).not.toBe(1);
    expect(FakeMeshBuilder.meshes.has('babylon:grid-line-x--10')).toBe(false);

    runtime.destroy();
  });

  it('ignores direct non-solid runtime object requests', () => {
    const runtime = createBabylonRuntime(createFakeBabylon());
    const host = document.createElement('div');
    ...
    expect(FakeMeshBuilder.meshes.size).toBe(0);
    expect(FakeDynamicTexture.instances).toHaveLength(0);
    expect(FakeTexture.instances).toHaveLength(0);
    expect(runtime.pick({ x: 10, y: 20 })).toBeNull();

    runtime.destroy();
  });

  it('rejects 3D text at the Babylon backend boundary', () => {
    ...
    expect(backend.create(node)).toMatchObject({
      ok: false,
      diagnostics: [{
        code: 'backend.unsupported-object',
  ...
  it('rejects equations but accepts surface wireframes through the backend contract', () => {
    const runtime = createBabylonRuntime(createFakeBabylon());
    const backend = createBabylonGraphBackend({ id: 'babylon', runtime });
    const host = document.createElement('div');
    ...
    const equation = backend.create(equationNode);
    ...
    expect(equation.diagnostics).toEqual([expect.objectContaining({ code: 'backend.unsupported-object' })]);
    expect(surface.diagnostics).toEqual([]);
    expect(scene.snapshot().handles.map((handle) => handle.objectId)).toEqual(['surface']);
    expect(FakeMeshBuilder.meshes.has('babylon:eq:segment-1')).toBe(false);
